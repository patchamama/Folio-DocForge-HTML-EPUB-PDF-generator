...
export class Sandbox {
  static async create(options) {
    ...
    frame.srcdoc = `<!doctype html><head><meta charset=utf8><base href="${document.baseURI}"></head><body>`
    if (options.place) {
      options.place(frame)
    } else {
  ...
  async setHTML(code, output) {
    ...
    if (loc != "about:blank" && loc != String(document.location) && !/\/empty\.html$/.test(loc)) {
      this.frame.srcdoc = `<!doctype html><head><meta charset=utf8><base href="${document.baseURI}"></head><body>`
      await awaitEvent(this.frame, "load")
      this.setupEnv()
