...
function saveVideoFile(filename, url, divContent, videoDir, inputDir = null) {
  ...
  const b64Match = /\bsrc=["']data:video\/[^;]+;base64,([A-Za-z0-9+/=\s]+)["']/.exec(divContent)
  if (b64Match) {
    try {
  ...
  if (url && inputDir && !/^https?:\/\//i.test(url)) {
    ...
    if (existsSync(localPath)) {
      ...
      console.warn(`  ⚠ video not found locally: ${localPath}`)
    }
  }

  // Try 3: download from absolute CDN URL via curl
  if (url && /^https?:\/\//i.test(url)) {
    try {
      mkdirSync(videoDir, { recursive: true })
      console.log(`  → video (download): ${filename}`)
      execSync(`curl -L --silent --max-time 120 -o "${videoPath}" "${url}"`, { stdio: 'pipe' })
...
function markVideoEmbeds(html, videoDir = null, inputDir = null) {
  ...
  //    then look backward for the enclosing vjs container div.
  {
    ...
    const VJS_PREFIXES = ['<div id=&quot;vjs_video_', '<div id="vjs_video_']
  ...
  //    Extract CDN URL and poster URL from the injected <a href> / <img src>.
  {
    ...
    while (true) {
      ...
      const filename = cdnUrl.replace(/\?.*$/, '').split('/').pop()
      const divEnd = findMatchingClose(result, '<div', '</div>', divStart)
...
function patchHtmlVideoPlayers(html) {
  ...
  if (html.includes('id="vjs_video_') || html.includes('id=&quot;vjs_video_')) {
    const srcRe = /data-savepage-src=(?:&quot;|")([^"&]+\.(?:mp4|webm|ogg|mov|avi)(?:\?[^"&]*)?)[&"]/gi
    const VJS_PREFIXES = ['<div id=&quot;vjs_video_', '<div id="vjs_video_']
